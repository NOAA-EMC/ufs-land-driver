...
contains   
  ...
  subroutine InitAllocate(this, namelist, vector_length)
    ...
    allocate(this%model%vegetation_fraction     (vector_length)            ) ! vegetation fraction [0.0-1.0]
  ...
  subroutine InitDefault(this)
    ...
    this%static%vegetation_category    = huge(1)
    this%static%soil_category          = huge(1)
    this%static%slope_category         = huge(1)
    this%static%soil_interface_depth   = huge(1.0)
    this%static%ice_flag               = huge(1)
    this%static%surface_type           = huge(1)
    this%static%crop_type              = huge(1)
    this%static%temperature_soil_bot   = huge(1.0)

    this%model%latitude                = huge(1.0)
    this%model%cosine_zenith           = huge(1.0)
    this%model%forcing_height          = huge(1.0)
    this%model%vegetation_fraction     = huge(1.0)
    this%model%max_vegetation_frac     = huge(1.0)
    this%model%snow_levels             = huge(1.0)
    this%model%interface_depth         = huge(1.0)
    this%model%snow_soil_thickness     = huge(1.0)
    this%model%leaf_area_index         = huge(1.0)
    this%model%stem_area_index         = huge(1.0)
    this%model%growing_deg_days        = huge(1.0)
    this%model%plant_growth_stage      = huge(1)
    this%model%cm_noahmp               = huge(1.0)
    this%model%ch_noahmp               = huge(1.0)
    this%model%ch_vegetated            = huge(1.0)
    this%model%ch_bare_ground          = huge(1.0)
    this%model%ch_leaf                 = huge(1.0)
    ...
    this%forcing%precip_non_convective = huge(1.0)
    this%forcing%precip_snow           = huge(1.0)
    this%forcing%precip_graupel        = huge(1.0)
    this%forcing%precip_hail           = huge(1.0)
    this%forcing%snowfall              = huge(1.0)
    this%forcing%rainfall              = huge(1.0)

    this%diag%z0_total                 = huge(1.0)
    this%diag%albedo_total             = huge(1.0)
    this%diag%albedo_direct            = huge(1.0)
    this%diag%albedo_diffuse           = huge(1.0)
    this%diag%albedo_direct_snow       = huge(1.0)
    this%diag%albedo_diffuse_snow      = huge(1.0)
    this%diag%emissivity_total         = huge(1.0)
    this%diag%canopy_gap_fraction      = huge(1.0)
    this%diag%incanopy_gap_fraction    = huge(1.0)
    this%diag%precip_frozen_frac       = huge(1.0)
    this%diag%snow_cover_fraction      = huge(1.0)
    this%diag%vegetation_fraction      = huge(1.0)
    this%diag%canopy_wet_fraction      = huge(1.0)
    this%diag%canopy_water             = huge(1.0)
    this%diag%depth_water_table        = huge(1.0)
    this%diag%lai_sunlit               = huge(1.0)
    this%diag%lai_shaded               = huge(1.0)
    this%diag%snow_ice_frac_old        = huge(1.0)
    this%diag%snow_albedo_old          = huge(1.0)
    this%diag%evaporation_potential    = huge(1.0)
    this%diag%soil_moisture_total      = huge(1.0)
    this%diag%temperature_veg_2m       = huge(1.0)
    this%diag%temperature_bare_2m      = huge(1.0)
    this%diag%temperature_2m           = huge(1.0)
    this%diag%spec_humidity_veg_2m     = huge(1.0)
    this%diag%spec_humidity_bare_2m    = huge(1.0)
    this%diag%spec_humidity_2m         = huge(1.0)
    this%diag%spec_humidity_surface    = huge(1.0)

    this%state%temperature_soil        = huge(1.0)
    this%state%temperature_snow        = huge(1.0)
    this%state%temperature_canopy_air  = huge(1.0)
    this%state%temperature_radiative   = huge(1.0)
    this%state%temperature_leaf        = huge(1.0)
    this%state%temperature_ground      = huge(1.0)
    this%state%temperature_bare_grd    = huge(1.0)
    this%state%temperature_veg_grd     = huge(1.0)
    this%state%vapor_pres_canopy_air   = huge(1.0)
    this%state%soil_liquid_vol         = huge(1.0)
    this%state%soil_moisture_vol       = huge(1.0)
    this%state%snow_water_equiv        = huge(1.0)
    this%state%snow_level_ice          = huge(1.0)
    this%state%snow_level_liquid       = huge(1.0)
    this%state%canopy_liquid           = huge(1.0)
    this%state%canopy_ice              = huge(1.0)
    this%state%aquifer_water           = huge(1.0)
    this%state%saturated_water         = huge(1.0)
    this%state%lake_water              = huge(1.0)
    this%state%soil_moisture_wtd       = huge(1.0)
    this%state%eq_soil_water_vol       = huge(1.0)
    this%state%leaf_carbon             = huge(1.0)
    this%state%root_carbon             = huge(1.0)
    this%state%stem_carbon             = huge(1.0)
    this%state%wood_carbon             = huge(1.0)
    this%state%soil_carbon_stable      = huge(1.0)
    this%state%soil_carbon_fast        = huge(1.0)
    this%state%grain_carbon            = huge(1.0)
    this%state%foliage_nitrogen        = huge(1.0)
    this%state%snow_water_equiv_old    = huge(1.0)
    this%state%snow_depth              = huge(1.0)
    this%state%snow_age                = huge(1.0)

    this%flux%sw_absorbed_total        = huge(1.0)
    this%flux%sw_reflected_total       = huge(1.0)
    this%flux%lw_absorbed_total        = huge(1.0)
    this%flux%sensible_heat_total      = huge(1.0)
    this%flux%transpiration_heat       = huge(1.0)
    this%flux%latent_heat_canopy       = huge(1.0)
    this%flux%latent_heat_ground       = huge(1.0)
    this%flux%latent_heat_total        = huge(1.0)
    this%flux%ground_heat_total        = huge(1.0)
    this%flux%precip_adv_heat_total    = huge(1.0)
    this%flux%sw_absorbed_veg          = huge(1.0)
    this%flux%sw_absorbed_ground       = huge(1.0)
    this%flux%lw_absorbed_grd_veg      = huge(1.0)
    this%flux%lw_absorbed_leaf         = huge(1.0)
    this%flux%lw_absorbed_grd_bare     = huge(1.0)
    this%flux%sensible_heat_grd_veg    = huge(1.0)
    this%flux%sensible_heat_leaf       = huge(1.0)
    this%flux%sensible_heat_grd_bar    = huge(1.0)
    this%flux%latent_heat_trans        = huge(1.0)
    this%flux%latent_heat_leaf         = huge(1.0)
    this%flux%latent_heat_grd_veg      = huge(1.0)
    this%flux%latent_heat_grd_bare     = huge(1.0)
    this%flux%snow_sublimation         = huge(1.0)
    this%flux%ground_heat_veg          = huge(1.0)
    this%flux%ground_heat_bare         = huge(1.0)
    this%flux%precip_adv_heat_veg      = huge(1.0)
    this%flux%precip_adv_heat_grd_v    = huge(1.0)
    this%flux%precip_adv_heat_grd_b    = huge(1.0)
    this%flux%transpiration            = huge(1.0)
    this%flux%evaporation_canopy       = huge(1.0)
    this%flux%evaporation_soil         = huge(1.0)
    this%flux%runoff_surface           = huge(1.0)
    this%flux%runoff_baseflow          = huge(1.0)
    this%flux%snowmelt_out             = huge(1.0)
    this%flux%snowmelt_shallow         = huge(1.0)
    this%flux%snowmelt_shallow_1       = huge(1.0)
    this%flux%snowmelt_shallow_2       = huge(1.0)
    this%flux%deep_recharge            = huge(1.0)
    this%flux%recharge                 = huge(1.0)
    this%flux%par_absorbed             = huge(1.0)
    this%flux%photosynthesis           = huge(1.0)
    this%flux%net_eco_exchange         = huge(1.0)
    this%flux%global_prim_prod         = huge(1.0)
    this%flux%net_prim_prod            = huge(1.0)
