...
p_in = '/dev/shm/lixu/noahmp/'
p_out = '/cpc/nidis/ufsland/uldas/output/'
...
def do_day2(yyyymmdd='20200101', n=365, m=1):
    '''daemon like routine'''
    ...
    for _ in range(10):
        ...
        for i in range(n):
            date = ndates(i * 24, yyyymmdd, fmt='%Y-%m-%d')
            fin = f'{p_in}/ufs_land_output*{date}*.nc'
            files = glob(fin)
...
if __name__ == '__main__':
    ...
    import fire
    if len(sys.argv) <= 1:
        print('input: YYYYMMDD nday nsec(1) or -h')
    else:
        if len(sys.argv[1]) == 8:
